...
//			System.out.println(loginUserService.findAll());
	...
	private void history29() {
		createLoginUserAsEnterprise("enterprise@gmail.com", "123", UserRole.ENTERPRISE);
		loginUserService.findAll().forEach(e -> System.out.println(e.getId()));
		createEnterprise("super acai", 2l);
		createOffer("compre 10 ganhe 1", "na compra de 10 acais o proximo eh de graca :)", 10, 3);
	}
	...
	private void createFinalClient(String uniqueIdentifier, String email) {
		FinalClient finalClient = FinalClient.builder().uniqueIdentifier(uniqueIdentifier).email(email).build();
		finalClientService.save(finalClient);
	}

	private void createOffer(String name, String description, int quantity, long enterpriseId) {
		Enterprise enterprise = enterpriseService.findById(enterpriseId);
		Offer offer = Offer.builder().name(name).description(description).quantity(quantity).enterprise(enterprise)
				.build();
	...
	private void createEnterprise(String enterpriseName, long finalCLientId) {
		...
		Enterprise enterprise = Enterprise.builder().name(enterpriseName).finalClients(finalClients).build();
		enterpriseService.save(enterprise);
	}

	private void createLoginUserAsEnterprise(String email, String password, UserRole userRole) {
		LoginUser loginUser1 = LoginUser.builder().email(email).password(password).userRole(userRole).build();
		loginUserService.save(loginUser1);
	}
}
